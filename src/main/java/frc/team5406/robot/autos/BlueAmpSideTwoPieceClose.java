...
public class BlueAmpSideTwoPieceClose extends SequentialCommandGroup {
  /** Creates a new Leave. */
  ...
  public BlueAmpSideTwoPieceClose(DriveSubsystem drive, ArmSubsystem arm, ShooterSubsystem shooter, FeederSubsystem feeder, VisionSubsystem camera, IntakeSubsystem intake, Supplier<AprilTag> targetSupplier) {
    this.arm = arm;
    ...
    List<Pose2d> path = List.of(
      ...
      new Pose2d(new Translation2d(2.9, 7.0), Rotation2d.fromDegrees(20))
    );
    PathConstraints constraints = new PathConstraints(3.0, 3.0, 4 * Math.PI, 2 * Math.PI);
    GoalEndState end = new GoalEndState(0.0,  Rotation2d.fromDegrees(25), true);
