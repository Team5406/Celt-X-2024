...
public class VisionSubsystem extends SubsystemBase implements AutoCloseable {
  public static class Hardware {
    ...
    ObjectDetection objectCamera;

    public Hardware(ObjectDetection objectCamera, AprilTagCamera... cameras) {
  ...
  private AtomicReference<HashMap<Integer, PhotonTrackedTarget>> m_visibleTags;

  private ObjectDetection m_objectCamera;
  ...
  private VisionSubsystem(Hardware visionHardware) {
    ...
    this.m_visibleTags = new AtomicReference<HashMap<Integer, PhotonTrackedTarget>>();
  ...
  private static Hardware initializeHardware() {
    ...
    HashMap<Integer, PhotonTrackedTarget> visibleTags= new HashMap<Integer, PhotonTrackedTarget>();
    for (var camera : m_apriltagCameras) {
      var result = camera.getLatestPipelineResult();
      if (result == null) continue;
      result.targets.forEach((photonTrackedTarget) -> {
        if (photonTrackedTarget.getFiducialId() == -1) return;
        visibleTagIDs.add(photonTrackedTarget.getFiducialId());
        visibleTags.put(photonTrackedTarget.getFiducialId(), photonTrackedTarget);
      });
    }
  ...
  public HashMap<Integer, PhotonTrackedTarget> getVisibleTags() {
  ...
  public boolean hasTarget(int desiredId) {
    List<Integer> visibleTagIDs = getVisibleTagIDs();
    return visibleTagIDs!=null && visibleTagIDs.contains(desiredId);
  }
