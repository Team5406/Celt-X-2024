...
public class ShooterSubsystem extends SubsystemBase {
  ...
  SimpleMotorFeedforward shooterFFLeft = new SimpleMotorFeedforward(Constants.ShooterHardware.SHOOTER_KS_LEFT, Constants.ShooterHardware.SHOOTER_KV_LEFT, Constants.ShooterHardware.SHOOTER_KA_LEFT);
  SimpleMotorFeedforward shooterFFRight = new SimpleMotorFeedforward(Constants.ShooterHardware.SHOOTER_KS_RIGHT, Constants.ShooterHardware.SHOOTER_KV_RIGHT, Constants.ShooterHardware.SHOOTER_KA_RIGHT);
  ...
  public void setShooterSpeed(double RPM, boolean same) {
    if (RPM == 0) {
      ...
      double arbFFLeft = shooterFFLeft.calculate(ShooterHardware.SHOOTER_SPEED_MULTIPLIER * RPM / 60.0);
      shooterMotorLeftOne.set(ShooterHardware.SHOOTER_SPEED_MULTIPLIER * RPM, ControlType.kVelocity, arbFFLeft, SparkPIDController.ArbFFUnits.kVoltage);
      }
    }
  }
