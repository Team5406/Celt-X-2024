...
public class BlueSourceSideTwoPieceOneFar extends SequentialCommandGroup {
  /** Creates a new Leave. */
  ...
  public BlueSourceSideTwoPieceOneFar(DriveSubsystem drive, ArmSubsystem arm, ShooterSubsystem shooter, FeederSubsystem feeder, VisionSubsystem camera, IntakeSubsystem intake, Supplier<AprilTag> targetSupplier) {
    this.arm = arm;
    ...
    List<Pose2d> path = List.of(
      ...
      new Pose2d(new Translation2d(3.7,1.0), Rotation2d.fromDegrees(0.0)),
      new Pose2d(new Translation2d(8.1, 1.0), Rotation2d.fromDegrees(180)),
      new Pose2d(new Translation2d(4.5, 1.5), Rotation2d.fromDegrees(140))

    );
    PathConstraints constraints = new PathConstraints(3.0, 3.0, 4 * Math.PI, 2 * Math.PI);
    GoalEndState end = new GoalEndState(0.0,  Rotation2d.fromDegrees(-30));
